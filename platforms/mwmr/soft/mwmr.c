			  ...
			  unsigned int no, const mwmr_t *mwmr )
{
	...
	soclib_io_set( coproc, MWMR_CONFIG_STATE_ADDR, &mwmr->usage );
	soclib_io_set( coproc, MWMR_CONFIG_OFFSET_ADDR,
				   (way == MWMR_TO_COPROC)
				   ? &mwmr->readp
				   : &mwmr->writep );
	soclib_io_set( coproc, MWMR_CONFIG_LOCK_ADDR, &mwmr->lock );
	soclib_io_set( coproc, MWMR_CONFIG_DEPTH, mwmr->gdepth );
	soclib_io_set( coproc, MWMR_CONFIG_WIDTH, mwmr->width );
	soclib_io_set( coproc, MWMR_CONFIG_BASE_ADDR, mwmr->buffer );
