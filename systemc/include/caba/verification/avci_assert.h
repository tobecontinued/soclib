...
class AdvancedVciAssert : public soclib::caba::BaseModule {
  ...
  private:
   ...
   class Packet {
     ...
     private:
      class Out {};
      Packet(const VciSignals<vci_param>& observedSignals, Out)
         :  cmd(0), address(0), contig(0), plen(0),
            srcid(observedSignals.rsrcid), trdid(observedSignals.rtrdid), pktid(observedSignals.rpktid) {}
      friend class PacketsList;
      
     public:
      class In {};
      Packet(const VciSignals<vci_param>& observedSignals, In)
         :  cmd(observedSignals.cmd), address(observedSignals.address), contig(observedSignals.contig), plen(observedSignals.plen),
            srcid(observedSignals.srcid), trdid(observedSignals.trdid), pktid(observedSignals.pktid) {}
   ...
   class PacketsList {
     ...
     public:
      ...
      void add(const VciSignals<vci_param>& observedSignals)
         {  lpContent.insert(new Packet(observedSignals, typename Packet::In())); }
      Packet* remove(const VciSignals<vci_param>& observedSignals)
         {  Packet pLocate(observedSignals, typename Packet::Out());
...
#include "caba/verification/avci_assert.h"
...
int sc_main(int ac, char *av[]) {
  ...
  vciAssert.observedSignals(vciSignals);
  vciAssert.observedSignals(vciSignalsVerif);
