...
#undef __ren
  ...
  private:
   ...
   class Packet {
     ...
     public:
      Packet(const In& in) : cmd(in.cmd), address(in.address), contig(in.contig), plen(in.plen) {}
   ...
   void acquireRequestCell()
      {  if (uRequestCells == 0) {
            ...
            packetCons = in.cons;
            plPendingPackets.add(Packet(in));
            ...
            else if (packetCmd == 2) { // Write
               if (packetContig)
                  laLockedAddresses.remove(packetAddress, packetPlen);
               else
                  laLockedAddresses.remove(packetAddress);
            };
         }
         else {
