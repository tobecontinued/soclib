...
#include "../include/iss_c6x_wrapper.h"
#include "xcache_signals.h"
#include "static_assert.h"

namespace soclib { namespace caba {

#define tmpl(x) template<typename iss_t> x IssC6xWrapper<iss_t>

tmpl(/**/)::IssC6xWrapper( sc_module_name insname, int ident )
	: BaseModule(insname),
      p_icache("icache"),
      p_dcache("dcache"),
      p_resetn("resetn"),
...
tmpl(void)::transition()
{
    ...
    bool frozen = m_iss.isBusy() || p_icache.frz.read() || dfrz || !m_iss.isInstPacketReady();
...
{
    {
        ...
        if (m_iss.isInstPacketReady())
            m_iss.setInstructionPacket();
    }
    {
        bool mem_asked;
        enum soclib::common::IssC6x::DataAccessType mem_type;
        uint32_t mem_addr;
        uint32_t mem_wdata;
