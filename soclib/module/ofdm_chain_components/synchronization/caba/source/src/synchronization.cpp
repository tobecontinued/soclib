...
#define tmpl(x) template<typename vci_param> x Synchronization<vci_param>
...
tmpl(void)::do_synchronization()
{
	...
	while(1)   {
	   ...
	   if (abs(accu) > So) {      
	      ...
	      // vider la memoire
	      for (int k=0; k<N; k++)
		 m_output_buffer = delay[k];
	      // acquisition de la trame 
	      for (int l= l+1; l<l+nS*N; l++)
		 m_output_buffer = m_input_buffer[l]; // mettre en sortie apres lecture d'une donnée
	   }
	   l++;
	}
}
