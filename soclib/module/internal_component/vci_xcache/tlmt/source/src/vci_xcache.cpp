...
#define tmpl(x) template<typename iss_t, typename vci_param> x VciXcache<iss_t,vci_param>
			    ...
			    const tlmt_core:: tlmt_time & time, void *private_data)
  {
    ...
    if(!m_vci_write){
      ...
      //std::cout << "XCACHE RECEIVE REPONSE time = " << std::dec << time << std::endl;
      update_time(time);
    }
  ...
  tmpl (/**/)::VciXcache(sc_core::sc_module_name name,
    ...
    : soclib::tlmt::BaseModule (name),
      ...
      p_vci("vci", new tlmt_core::tlmt_callback<VciXcache,vci_rsp_packet<vci_param>*>(this,&VciXcache<iss_t,vci_param>::rspReceived),&c0)
  {
    ...
    for (int32_t i = 0 ; i < iss_t::n_irq ; i++) {
      ...
      new(&p_irq[i])tlmt_core::tlmt_in<bool> (o.str(), 
					      new tlmt_core::tlmt_callback<VciXcache, bool>(this,&VciXcache<iss_t, vci_param>::irqReceived, (void*)(long)i));
    }
