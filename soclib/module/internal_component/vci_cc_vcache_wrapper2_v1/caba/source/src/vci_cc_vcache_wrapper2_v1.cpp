        ...
        case DCACHE_IDLE:
          {
            ...
            else if (dreq.valid) 
            {
              ...
              else                                                            // MMU activated
              {
                ...
                //std::cout << "got way " << dcache_tlb_way << " set " << dcache_tlb_set << " for VA " << dreq.addr << std::endl;
                ...
                switch( dreq.type ) {
                  ...
                  case iss_t::DATA_WRITE:
                    ...
                    else if ( !dcache_pte_info.d && (r_mmu_mode.read() & DATA_TLB_MASK) )   // dirty bit update required
                    {
                      ...
                      {
                        uint32_t val;
                        val = dcache_tlb.getpte(dcache_tlb_way, dcache_tlb_set) | PTE_D_MASK;
                        r_dcache_pte_update = val;
                        r_dcache_tlb_paddr = (paddr_t)r_mmu_ptpr << (INDEX1_NBITS+2) | (paddr_t)((dreq.addr>>PAGE_M_NBITS)<<2);
                        r_dcache_tlb_ll_dirty_req = true;
                        r_dcache_fsm = DCACHE_LL_DIRTY_WAIT;
                        m_cpt_data_tlb_write_dirty++;
                        //std::cout << "switch to DCACHE_LL_DIRTY_WAIT wr " << val << "@" << r_dcache_tlb_paddr << " tblp " << dcache_tlb_way << " " << dcache_tlb_set << std::endl;
                      }
                      ...
                      {   
                        if (dcache_hit_p) 
                        {
                          ...
                          r_dcache_tlb_ll_dirty_req = true;
                          r_dcache_fsm = DCACHE_LL_DIRTY_WAIT;
                          //std::cout << "switch to DCACHE_LL_DIRTY_WAIT(2) wr " << r_dcache_pte_update << "@" << r_dcache_tlb_paddr << std::endl;
                          m_cpt_data_tlb_write_dirty++;
                        }
                        else    // get PTBA to calculate the physical address of PTE
                        {
                          ...
                          r_dcache_fsm = DCACHE_DTLB1_READ_CACHE;
                          //std::cout << "switch to DCACHE_DTLB1_READ_CACHE wr " << r_dcache_pte_update << "@" << r_dcache_tlb_paddr << std::endl;
                        }
                      }
                      m_cost_data_tlb_miss_frz++;
                    }
              ...
              else    // cached write
              {
                ...
                else if (!r_dcache_dirty_save && (r_mmu_mode.read() & DATA_TLB_MASK))   // dirty bit update required
                {
                  if (dcache_tlb.getpagesize(r_dcache_tlb_way_save, r_dcache_tlb_set_save)) 
                  {
                    ...
                    r_dcache_tlb_ll_dirty_req = true;
                    r_dcache_fsm = DCACHE_LL_DIRTY_WAIT;
                    //std::cout << "switch to DCACHE_LL_DIRTY_WAIT(3) wr " << r_dcache_pte_update << "@" << r_dcache_tlb_paddr << std::endl;
                    m_cpt_data_tlb_write_dirty++;
                  }
                  else
                  {   
                    if (r_dcache_hit_p_save) 
                    {
                      ...
                      r_dcache_fsm = DCACHE_LL_DIRTY_WAIT;
                      m_cpt_data_tlb_write_dirty++;
                      //std::cout << "switch to DCACHE_LL_DIRTY_WAIT(4) wr " << r_dcache_pte_update << "@" << r_dcache_tlb_paddr << std::endl;
                    }
                    else
                    {
                      r_dcache_pte_update = dcache_tlb.getpte(r_dcache_tlb_way_save, r_dcache_tlb_set_save) | PTE_D_MASK;
                      ...
                      r_dcache_tlb_ptba_read = true;
                      r_dcache_fsm = DCACHE_DTLB1_READ_CACHE;
                      //std::cout << "switch to DCACHE_DTLB1_READ_CACHE(2) wr " << r_dcache_pte_update << "@" << r_dcache_tlb_paddr << std::endl;
                    }
                  }
                  m_cost_data_tlb_miss_frz++;
                }
            ...
            if (!r_dcache_tlb_ll_dirty_req)
            {
              ...
              else
              {
                ...
                else
                {
                  ...
                  //std::cout << "switch to DCACHE_SC_DIRTY_WAIT wr " << r_dcache_pte_update << " " << r_dcache_miss_buf[0] << " " << r_dcache_pte_update.read() << std::endl;
                }
              }
            }
            break;
              ...
              else	// PTE
              {
                ...
                if ( (m_srcid_rw >> 4) == ((r_dcache_tlb_paddr.read() & ((1<<(m_paddr_nbits - PAGE_M_NBITS))-1)) >> (m_paddr_nbits - PAGE_M_NBITS -10)) ) // local
                {
                  if ( (tlb_data & PTE_L_MASK ) >> PTE_L_SHIFT ) // L bit is set
                  {
                    ...
                    //std::cout << "switch to DCACHE_TLB1_UPDT wr " << r_dcache_pte_update << "@" << r_dcache_tlb_paddr << std::endl;
                  }
                  else
                  {
                    ...
                    m_cpt_ins_tlb_write_et++;
                    //std::cout << "switch to DCACHE_TLB1_LL_WAIT wr " << r_dcache_pte_update << "@" << r_dcache_tlb_paddr << std::endl;
                  }
                }
                else // remotely
                {
                  if ( (tlb_data & PTE_R_MASK ) >> PTE_R_SHIFT ) // R bit is set
                  {
                    r_dcache_pte_update = tlb_data;
                    r_dcache_fsm = DCACHE_TLB1_UPDT;
                    //std::cout << "switch to DCACHE_TLB1_UPDT(2) wr " << r_dcache_pte_update << "@" << r_dcache_tlb_paddr << std::endl;
                  }
                  else
                  {
                    ...
                    assert(write_hit && "Write on miss ignores data");  
                    r_dcache_fsm = DCACHE_TLB1_LL_WAIT;
                    //std::cout << "switch to DCACHE_TLB1_LL_WAIT(2) wr " << r_dcache_pte_update << "@" << r_dcache_tlb_paddr << std::endl;
                    m_cpt_ins_tlb_write_et++;
                  }
                }
              }
            ...
            if (!r_dcache_tlb_ll_acc_req)
            {
              ...
              else
              {
                ...
                else
                {
                  ...
                  //std::cout << "switch to DCACHE_TLB1_SC_WAIT wr " << r_dcache_pte_update << "@" << r_dcache_tlb_paddr << std::endl;
                }
              }
            }
            break;
              ...
              else	// PTE
              {
                ...
                if ( (m_srcid_rw >> 4) == ((r_dcache_tlb_paddr.read() & ((1<<(m_paddr_nbits - PAGE_M_NBITS))-1)) >> (m_paddr_nbits - PAGE_M_NBITS -10)) ) // local
                {
                  if ( (rsp_dtlb_miss & PTE_L_MASK ) >> PTE_L_SHIFT ) // L bit is set
                  {
                    ...
                    r_dcache_fsm        = DCACHE_TLB1_UPDT;
                    //std::cout << "switch to DCACHE_TLB1_UPDT(3) wr " << r_dcache_pte_update << "@" << r_dcache_tlb_paddr << std::endl;
                  }
                  else
                  {
                    ...
                    assert(write_hit && "Write on miss ignores data");  
                    r_dcache_fsm        = DCACHE_TLB1_LL_WAIT;
                    //std::cout << "switch to DCACHE_TLB1_LL_WAIT(3) wr " << r_dcache_pte_update << "@" << r_dcache_tlb_paddr << std::endl;
                    m_cpt_ins_tlb_write_et++;
                  }
                }
                else // remotely
                {
                  if ( (rsp_dtlb_miss & PTE_R_MASK ) >> PTE_R_SHIFT ) // R bit is set
                  {
                    r_dcache_pte_update = rsp_dtlb_miss;
                    r_dcache_fsm        = DCACHE_TLB1_UPDT;
                    //std::cout << "switch to DCACHE_TLB1_UPDT(4) wr " << r_dcache_pte_update << "@" << r_dcache_tlb_paddr << std::endl;
                  }
                  else
                  {
                    ...
                    assert(write_hit && "Write on miss ignores data");  
                    r_dcache_fsm        = DCACHE_TLB1_LL_WAIT;
                    //std::cout << "switch to DCACHE_TLB1_LL_WAIT(4) wr " << r_dcache_pte_update << "@" << r_dcache_tlb_paddr << std::endl;
                    m_cpt_ins_tlb_write_et++;
                  }
                }
              }
        ...
        case DCACHE_DTLB2_READ_CACHE:
          {
            ...
            if ( tlb_hit_cache )
            {
              ...
              else if ( (tlb_data & PTE_T_MASK) >> PTE_T_SHIFT ) //PTD
              {
                ...
                //std::cout << "switch to DCACHE_TLB2_UPDT wr " << r_dcache_pte_update << "@" << r_dcache_tlb_paddr << std::endl;
              }
              else 
              {
                if ( (m_srcid_rw >> 4) == ((r_dcache_tlb_paddr.read() & ((1<<(m_paddr_nbits - PAGE_M_NBITS))-1)) >> (m_paddr_nbits - PAGE_M_NBITS -10)) ) // local
                {
                  if ( (tlb_data & PTE_L_MASK ) >> PTE_L_SHIFT ) // L bit is set
                  {
                    ...
                    r_dcache_ppn_update = tlb_data_ppn;
                    r_dcache_fsm        = DCACHE_TLB2_UPDT;
                    //std::cout << "switch to DCACHE_TLB2_UPDT(2) wr " << r_dcache_pte_update << "@" << r_dcache_tlb_paddr << std::endl;
                  }
                  else
                  {
                    r_dcache_pte_update = tlb_data | PTE_L_MASK;
                    ...
                    m_cpt_ins_tlb_write_et++;
                    //std::cout << "switch to DCACHE_TLB2_LL_WAIT wr " << r_dcache_pte_update << "@" << r_dcache_tlb_paddr << std::endl;
                  }
                }
                else // remotely
                {
                  ...
                  else
                  {
                    ...
                    assert(write_hit && "Write on miss ignores data");  
                    r_dcache_fsm = DCACHE_TLB2_LL_WAIT;
                    //std::cout << "switch to DCACHE_TLB2_LL_WAIT(2) wr " << r_dcache_pte_update << "@" << r_dcache_tlb_paddr << std::endl;
                    m_cpt_ins_tlb_write_et++;
                  }
                }
              }
            ...
            if (!r_dcache_tlb_ll_acc_req)
            {
              ...
              else
              {
                ...
                else
                {
                  ...
                  //std::cout << "switch to DCACHE_TLB2_SC_WAIT wr " << r_dcache_pte_update << "@" << r_dcache_tlb_paddr << std::endl;
                }
              }
            }
            break;
        ...
        case DCACHE_TLB2_READ_UPDT:
          {
            ...
            if ( !r_dcache_cleanup_req )
            {
              ...
              else if ( (rsp_dtlb_miss & PTE_T_MASK) >> PTE_T_SHIFT ) // PTD
              {
                ...
                r_dcache_ppn_update = tlb_data_ppn;
                r_dcache_fsm = DCACHE_TLB2_UPDT;
                //std::cout << "switch to DCACHE_TLB2_UPDT(4) wr " << r_dcache_pte_update << "@" << r_dcache_tlb_paddr << std::endl;
              }
              else
              {
                if ( (m_srcid_rw >> 4) == ((r_dcache_tlb_paddr.read() & ((1<<(m_paddr_nbits - PAGE_M_NBITS))-1)) >> (m_paddr_nbits - PAGE_M_NBITS -10)) ) // local
                {
                  if ( (rsp_dtlb_miss & PTE_L_MASK ) >> PTE_L_SHIFT ) // L bit is set
                  {
                    ...
                    r_dcache_ppn_update = tlb_data_ppn;
                    r_dcache_fsm        = DCACHE_TLB2_UPDT;
                    //std::cout << "switch to DCACHE_TLB2_UPDT(5) wr " << r_dcache_pte_update << "@" << r_dcache_tlb_paddr << std::endl;
                  }
                  else
                  {
                    r_dcache_pte_update = rsp_dtlb_miss | PTE_L_MASK;
                    ...
                    r_dcache_fsm = DCACHE_TLB2_LL_WAIT;
                    m_cpt_ins_tlb_write_et++;
                    //std::cout << "switch to DCACHE_TLB2_LL_WAIT(4) wr " << r_dcache_pte_update << "@" << r_dcache_tlb_paddr << std::endl;
                  }
                }
                else // remotely
                {
                  if ( (rsp_dtlb_miss & PTE_R_MASK ) >> PTE_R_SHIFT ) // R bit is set
                  {
                    ...
                    r_dcache_ppn_update = tlb_data_ppn;
                    r_dcache_fsm        = DCACHE_TLB2_UPDT;
                    //std::cout << "switch to DCACHE_TLB2_UPDT(6) wr " << r_dcache_pte_update << "@" << r_dcache_tlb_paddr << std::endl;
                  }
                  else
                  {
                    r_dcache_pte_update = rsp_dtlb_miss | PTE_R_MASK;
                    ...
                    assert(write_hit && "Write on miss ignores data");  
                    r_dcache_fsm = DCACHE_TLB2_LL_WAIT;
                    //std::cout << "switch to DCACHE_TLB2_LL_WAIT(6) wr " << r_dcache_pte_update << "@" << r_dcache_tlb_paddr << std::endl;
                    m_cpt_ins_tlb_write_et++;
                  }
                }
              }
        ...
        case DCACHE_WRITE_UPDT:
          {
            ...
            if ( !r_dcache_dirty_save && (r_mmu_mode.read() & DATA_TLB_MASK) )   
            {
              if ( dcache_tlb.getpagesize(r_dcache_tlb_way_save, r_dcache_tlb_set_save) )	// 2M page size, one level page table 
              {
                ...
                r_dcache_tlb_ll_dirty_req = true;
                r_dcache_fsm = DCACHE_LL_DIRTY_WAIT;
                //std::cout << "switch to DCACHE_LL_DIRTY_WAIT(6) wr " << r_dcache_pte_update << "@" << r_dcache_tlb_paddr << std::endl;
                m_cpt_data_tlb_write_dirty++;
              }
              else
              {   
                if (r_dcache_hit_p_save) 
                {
                  ...
                  r_dcache_fsm = DCACHE_LL_DIRTY_WAIT;
                  m_cpt_data_tlb_write_dirty++;
                  //std::cout << "switch to DCACHE_LL_DIRTY_WAIT(7) wr " << r_dcache_pte_update << "@" << r_dcache_tlb_paddr << std::endl;
                }
                else
                {
                  r_dcache_pte_update = dcache_tlb.getpte(r_dcache_tlb_way_save, r_dcache_tlb_set_save) | PTE_D_MASK;
