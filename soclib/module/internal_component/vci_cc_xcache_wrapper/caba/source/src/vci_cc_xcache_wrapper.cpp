...
/*
    r_icache_miss_buf = 
        soclib::common::alloc_elems<sc_signal<data_t> >("icache_miss_buff", icache_words);;
    r_dcache_miss_buf = 
        soclib::common::alloc_elems<sc_signal<data_t> >("dcache_miss_buff", dcache_words);
    r_tgt_buf =         
        soclib::common::alloc_elems<sc_signal<data_t> >("tgt_buf" , dcache_words);
    r_tgt_val =        
        soclib::common::alloc_elems<sc_signal<bool> >("tgt_val" , dcache_words);
*/
...
std::cout << " tgt fsm: " << tgt_fsm_state_str[r_vci_tgt_fsm] << std::endl
    ...
    // - r_tgt_trdid
    // - r_tgt_pktid
    // All VCI commands must have the CMD_WRITE value in the CMD field.
    // If the VCI address offset is null, the command is an invalidate 
    // request. It is an update request otherwise.
    // The VCI_TGT FSM sets the r_tgt_icache_req & r_tgt_dcache_req
    // flip-flops to signal the external request to the ICACHE & DCACHE
    // FSM respectively.
    // 1/ for an invalidate request the VCI packet length is 1 word.
    // The WDATA field contains the line index (i.e. the Z & Y fields).
    // 2/ for an update request the VCI packet length is (n+2) words.
    ...
    // 3/ for both invalidate & update requests, the VCI response
    // is one single word.
    ...
    case TGT_UPDT_WORD:
        if (p_vci_tgt.cmdval.read()) {
            ...
            for ( size_t word = 0 ; word < m_dcache_words ; word++ ) r_tgt_val[word] = false;
    ...
    case TGT_UPDT_DATA:
        if (p_vci_tgt.cmdval.read()) {
            if ( (r_tgt_word + r_tgt_cpt - 2) >= m_dcache_words ) {
                std::cout << "error in component VCI_CC_XCACHE_WRAPPER " << name() << std::endl;
                std::cout << "the reveived UPDATE command length is wrong" << std::endl;
                exit(0);
            }
            size_t word = r_tgt_word + r_tgt_cpt - 2;
...
std::cout << " Instruction Request: " << ireq << std::endl;
    ...
    // default value for response to processor request
    irsp.error          = false;
    irsp.valid          = false;
    irsp.instruction    = 0;
    ...
    case ICACHE_UPDT:   // cache update after a MISS
    {
    ...
    case ICACHE_CC_INVAL:   // external invalidate request 
    {                       
...
std::cout << " Data Request: " << dreq << std::endl;
    ...
    // default value for the response to the processor request
    drsp.error = false;
    drsp.valid = false;
    drsp.rdata = 0;
...
std::cout << " Data Response: " << drsp << std::endl;
    ...
    case CMD_DATA_CLEANUP:
        if ( p_vci_ini.cmdack.read() ) r_vci_cmd_fsm = CMD_IDLE;
        break;
...
//////////////////////////////////////////////////////////////////////////////////
{
    ...
    case CMD_DATA_MISS:
        p_vci_ini.cmdval = true;
        p_vci_ini.address = (r_dcache_addr_save & m_dcache_yzmask) + (r_vci_cmd_cpt<<2) ;
        p_vci_ini.be     = 0xF;
        ...
        p_vci_ini.clen   = 0;
        p_vci_ini.cfixed = false;
        p_vci_ini.eop = ( r_vci_cmd_cpt == (m_dcache_words - 1) );
        break;

    case CMD_INS_MISS:
        p_vci_ini.cmdval = true;
        p_vci_ini.address = (r_icache_addr_save & m_icache_yzmask) + (r_vci_cmd_cpt<<2) ;
        p_vci_ini.be     = 0xF;
        ...
        p_vci_ini.clen   = 0;
        p_vci_ini.cfixed = false;
        p_vci_ini.eop = ( r_vci_cmd_cpt == (m_icache_words - 1) );
        break;
    ...
    switch ((tgt_fsm_state_e)r_vci_tgt_fsm.read()) {
