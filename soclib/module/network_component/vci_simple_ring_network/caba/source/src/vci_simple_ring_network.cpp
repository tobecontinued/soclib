...
#include "../include/vci_simple_ring_network.h"
...
#define tmpl(x) template<typename vci_param> x VciSimpleRingNetwork<vci_param>

////////////////////////////////
//      constructor
//
////////////////////////////////
tmpl(/**/)::VciSimpleRingNetwork( sc_module_name insname,
                           ...
                           m_nat(nb_attached_target)
{
	...
	for( int x = 0; x < m_nai ; x++ )
	{
		...
		t_target_wrapper[x] = new soclib::caba::VciRingTargetWrapper<vci_param>(o.str().c_str(), alloc_target, wrapper_fifo_depth, mt, ringid, x);
	}
	...
	for( int x = 0 ; x < m_nat-1 ; x++ )
	{
      ...
      // connexion derniere cible avec premier init
	t_target_wrapper[m_nat-1]->p_clk(p_clk);
	t_target_wrapper[m_nat-1]->p_resetn(p_resetn);
	t_target_wrapper[m_nat-1]->p_vci(p_to_target[m_nat-1]) ; 
	t_target_wrapper[m_nat-1]->p_ring_in(t_ring_signal[nbsig]);
	t_target_wrapper[m_nat-1]->p_ring_out(t_ring_signal[0]);          
}
  
//---------------- destructor
tmpl(/**/)::~VciSimpleRingNetwork()
{
