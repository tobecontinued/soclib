  ...
  sc_core::sc_time delay )                           // interconnect delay
{
  ...
  for (size_t i=0;i<nb_target;i++){
    ...
    m_CmdArbRspRout.push_back(new VciCmdArbRspRout(tmpName.str().c_str(), mt.getIdMaskingTable(0), delay));
  }
  ...
  for(size_t i=0;i<nb_init;i++){
    ...
    p_to_initiator.push_back(new tlm_utils::simple_target_socket_tagged<VciVgmn,32,tlm::tlm_base_protocol_types>(target_name.str().c_str()));
    p_to_initiator[i]->register_nb_transport_fw(this, &VciVgmn::nb_transport_fw_down, i);
  ...
  for(size_t i=0;i<nb_target;i++){
    ...
    p_to_target.push_back(new tlm_utils::simple_initiator_socket_tagged<VciVgmn,32,tlm::tlm_base_protocol_types>(init_name.str().c_str()));
    p_to_target[i]->register_nb_transport_bw(this, &VciVgmn::nb_transport_bw_down, i);
