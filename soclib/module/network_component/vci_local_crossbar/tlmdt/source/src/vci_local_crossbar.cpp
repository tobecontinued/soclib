  ...
  : m_centralized_buffer(++nb_init)                 // centralized buffer
{
  ...
  for (size_t i=0;i<nb_target;i++){
    ...
    if(i==(nb_target - 1)) // only the last CMD block has external access
      m_CmdArbRspRout.push_back(new VciCmdArbRspRout(tmpName.str().c_str(), mt, index, UNIT_TIME, true));
    else
      m_CmdArbRspRout.push_back(new VciCmdArbRspRout(tmpName.str().c_str(), mt, index, UNIT_TIME, false));
  }
  ...
  for (size_t i=0;i<nb_init;i++){
    ...
    m_RspArbCmdRout.push_back(new VciRspArbCmdRout(tmpName.str().c_str() ,mt, index, i, UNIT_TIME, &m_centralized_buffer));
  }
