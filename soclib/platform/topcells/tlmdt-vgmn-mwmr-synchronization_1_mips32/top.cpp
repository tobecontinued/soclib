...
int sc_main (int argc, char *argv[])
{
  ...
  xcache= new soclib::tlmdt::VciXcacheWrapper<vci_param, soclib::common::Iss2Simhelper<soclib::common::Mips32ElIss> >("misp0", 0, IntTab(0), maptab, 8, 4, 8, 4, 500*UNIT_TIME, simulation_time*UNIT_TIME);

  xcache->p_vci_initiator(vgmn_1.m_RspArbCmdRout[0]->p_vci_target);
  ...
  vgmn_1.m_CmdArbRspRout[1]->p_vci_initiator(ram1.p_vci_target);

  soclib::tlmdt::VciRam<vci_param> ram2("ram2", IntTab(4), maptab, loader);
  vgmn_1.m_CmdArbRspRout[4]->p_vci_initiator(ram2.p_vci_target);
 ...
 soclib::tlmdt::VciLocks<vci_param> vcilocks("vcilocks", IntTab(5), maptab);
  vgmn_1.m_CmdArbRspRout[5]->p_vci_initiator(vcilocks.p_vci_target);
 ...
 soclib::tlmdt::VciMwmrController<vci_param> mwmr0("mwmr0", maptab, IntTab(1), IntTab(3), read_depth, write_depth, n_read_channel, n_write_channel, n_config, n_status, simulation_time * UNIT_TIME);
  ...
  vgmn_1.m_CmdArbRspRout[3]->p_vci_initiator(mwmr0.p_vci_target);
  //std::cout<<"je suis ici8"<<std::endl;

  Synchronization synchronization("synchronization", 0, read_depth/4, write_depth/4, n_read_channel, n_write_channel, n_config, n_status);

  for(uint32_t i=0; i<n_read_channel; i++)
    (*mwmr0.p_read_fifo[i])(*synchronization.p_read_fifo[i]);

  for(uint32_t i=0; i<n_write_channel; i++)
    (*mwmr0.p_write_fifo[i])(*synchronization.p_write_fifo[i]);

  for(uint32_t i=0; i<n_config; i++)
    (*mwmr0.p_config[i])(*synchronization.p_config[i]);

  for(uint32_t i=0; i<n_status; i++)
    (*mwmr0.p_status[i])(*synchronization.p_status[i]);
  /////////////////////////////////////////////////////////////////////////////
  // SIMULATION
  /////////////////////////////////////////////////////////////////////////////
