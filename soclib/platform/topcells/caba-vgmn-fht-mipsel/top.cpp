...
int _main(int argc, char *argv[])
{
    ...
    // FHT
    ////////////////////////////////////////////////
    soclib::caba::fht fht("fht",_CLK_PERIOD);
    ...
    //FHT
    fht.p_clk(signal_clk);
    fht.p_resetn(signal_resetn);
    fht.p_from_MWMR[0](signal_mwmr_to_coproc0);
    fht.p_from_MWMR[1](signal_mwmr_to_coproc1);
    fht.p_to_MWMR[0](signal_mwmr_from_coproc0);
    fht.p_to_MWMR[1](signal_mwmr_from_coproc1);
    fht.p_core_config(signal_mwmr_config);
    for (int i=0; i<6; i++) {
        fht.p_config[i](signal_p_config[i]);          //Configuration registers
    }
    for (int i=0; i<5; i++) {
        fht.p_status[i](signal_p_status[i]);          //Status register
    }
