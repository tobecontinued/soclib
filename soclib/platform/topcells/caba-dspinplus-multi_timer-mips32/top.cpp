...
int _main(int argc, char *argv[])
{
	...
	maptab.add(Segment("reset", RESET_BASE, RESET_SIZE, IntTab(0x00), true));
	maptab.add(Segment("excep", EXCEP_BASE, EXCEP_SIZE, IntTab(0x00), true));
	maptab.add(Segment("text" , TEXT_BASE , TEXT_SIZE , IntTab(0x00), true));
  
	maptab.add(Segment("data" , DATA_BASE , DATA_SIZE , IntTab(0x00), true));
  
	maptab.add(Segment("loc0" , LOC0_BASE , LOC0_SIZE , IntTab(0x00), true));
	maptab.add(Segment("loc1" , LOC1_BASE , LOC1_SIZE , IntTab(0x00), true));
	maptab.add(Segment("loc2" , LOC2_BASE , LOC2_SIZE , IntTab(0x00), true));
	maptab.add(Segment("loc3" , LOC3_BASE , LOC3_SIZE , IntTab(0x00), true));
  
	maptab.add(Segment("tty"  , TTY_BASE  , TTY_SIZE  , IntTab(0x01), false));
	maptab.add(Segment("timer", TIMER_BASE, TIMER_SIZE, IntTab(0x10), false));
	maptab.add(Segment("locks", LOCKS_BASE, LOCKS_SIZE, IntTab(0x11), false));
	...
	soclib::caba::VciXcacheWrapper<vci_param, soclib::common::Iss2Simhelper<soclib::common::Mips32ElIss> > cache0("cache0", 0, maptab,IntTab(0x00), 4,1,8, 4,1,8);
	soclib::caba::VciXcacheWrapper<vci_param, soclib::common::Iss2Simhelper<soclib::common::Mips32ElIss> > cache1("cache1", 1, maptab,IntTab(0x01), 4,1,8, 4,1,8);
	soclib::caba::VciXcacheWrapper<vci_param, soclib::common::Iss2Simhelper<soclib::common::Mips32ElIss> > cache2("cache2", 2, maptab,IntTab(0x10), 4,1,8, 4,1,8);
	soclib::caba::VciXcacheWrapper<vci_param, soclib::common::Iss2Simhelper<soclib::common::Mips32ElIss> > cache3("cache3", 3, maptab,IntTab(0x11), 4,1,8, 4,1,8);

	soclib::common::ElfLoader loader("soft/bin.soft");
	soclib::caba::VciSimpleRam<vci_param> vcimultiram0("vcimultiram0", IntTab(0x00), maptab, loader);
	soclib::caba::VciMultiTty<vci_param> vcitty("vcitty",	IntTab(0x01), maptab, "vcitty0", "vcitty1", "vcitty2", "vcitty3", NULL);
	soclib::caba::VciTimer<vci_param> vcitimer("vcittimer", IntTab(0x10), maptab, 4);
	soclib::caba::VciLocks<vci_param> vcilocks("vcilocks", IntTab(0x11), maptab); 
	...
	vciDspinPlusNetwork.p_clk(signal_clk);
	vciDspinPlusNetwork.p_resetn(signal_resetn);
      
	vciDspinPlusNetwork.p_to_initiator[0][0](signal_vci_m0);
	vciDspinPlusNetwork.p_to_initiator[0][1](signal_vci_m1);
	vciDspinPlusNetwork.p_to_initiator[1][0](signal_vci_m2);
	vciDspinPlusNetwork.p_to_initiator[1][1](signal_vci_m3);

	vciDspinPlusNetwork.p_to_target[0][0](signal_vci_vcimultiram0);
	vciDspinPlusNetwork.p_to_target[0][1](signal_vci_tty);
	vciDspinPlusNetwork.p_to_target[1][0](signal_vci_vcitimer);
	vciDspinPlusNetwork.p_to_target[1][1](signal_vci_vcilocks);
