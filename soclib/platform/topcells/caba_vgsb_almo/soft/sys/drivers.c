...
/* Here, we can perform some static parameters checking */
#if !defined(NB_PROCS) || !defined(NB_MAXTASKS)
# error You must provide a definition for NB_PROCS and NB_MAXTASKS in 'config.h' file!
#endif
#if NB_PROCS > 8
 ...
 *   using the _tty_config() function. The actual stored value is tty_id + 0xFF000000
 *   A 0 value means that the default tty_id must be used. It is computed as :
 *       proc_id * NB_MAXTASKS * TTY_SPAN + task_id.
