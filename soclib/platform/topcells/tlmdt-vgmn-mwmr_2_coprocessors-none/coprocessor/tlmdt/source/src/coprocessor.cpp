	   ...
	   m_status_registers(n_status)
{
  ...
  for(uint32_t i=0;i<m_read_channels;i++){
    ...
    tmpName << "read_fifo" << i;
    p_read_fifo.push_back(new tlm_utils::simple_initiator_socket_tagged<Coprocessor,32,tlm::tlm_base_protocol_types>(tmpName.str().c_str()));
    p_read_fifo[i]->register_nb_transport_bw(this, &Coprocessor::readResponseReceived, i);
  }
  ...
  for(uint32_t i=0;i<m_write_channels;i++){
    ...
    p_write_fifo.push_back(new tlm_utils::simple_initiator_socket_tagged<Coprocessor,32,tlm::tlm_base_protocol_types>(tmpName.str().c_str()));
    p_write_fifo[i]->register_nb_transport_bw(this, &Coprocessor::writeResponseReceived, i);
  }
  ...
  for(uint32_t i=0;i<m_config_registers;i++){
    ...
    std::ostringstream tmpName;
    tmpName << "config" << i;
    p_config.push_back(new tlm_utils::simple_target_socket_tagged<Coprocessor,32,tlm::tlm_base_protocol_types>(tmpName.str().c_str()));
    p_config[i]->register_nb_transport_fw(this, &Coprocessor::writeConfigReceived, i);
  }
  ...
  for(uint32_t i=0;i<m_status_registers;i++){
    ...
    std::ostringstream tmpName;
    tmpName << "status" << i;
    p_status.push_back(new tlm_utils::simple_target_socket_tagged<Coprocessor,32,tlm::tlm_base_protocol_types>(tmpName.str().c_str()));
    p_status[i]->register_nb_transport_fw(this, &Coprocessor::readStatusReceived, i);
